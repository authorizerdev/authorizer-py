...
class AsyncAuthorizerClient:
    ...
    async def _oauth_form(self, path: str, body: dict[str, str]) -> dict[str, Any]:
    ...
    # -- MFA setup / recovery ---------------------------------------------- #
    async def skip_mfa_setup(
        self, req: t.SkipMfaSetupRequest, headers: dict[str, str] | None = None
    ) -> t.AuthToken:
        res = await self._invoke(
            "skip_mfa_setup", d.PUBLIC["skip_mfa_setup"], req.to_dict(), headers
        )
        return t.AuthToken.from_dict(res or {})
    ...
    async def lock_mfa(
        ...
        res = await self._invoke("lock_mfa", d.PUBLIC["lock_mfa"], req.to_dict(), headers)
        return t.GenericResponse.from_dict(res or {})

    async def email_otp_mfa_setup(
        self, req: t.OtpMfaSetupRequest | None = None, headers: dict[str, str] | None = None
    ) -> t.GenericResponse:
        data = req.to_dict() if req is not None else None
        res = await self._invoke(
            "email_otp_mfa_setup", d.PUBLIC["email_otp_mfa_setup"], data, headers
        )
        return t.GenericResponse.from_dict(res or {})

    async def sms_otp_mfa_setup(
        self, req: t.OtpMfaSetupRequest | None = None, headers: dict[str, str] | None = None
    ) -> t.GenericResponse:
        data = req.to_dict() if req is not None else None
        res = await self._invoke(
            "sms_otp_mfa_setup", d.PUBLIC["sms_otp_mfa_setup"], data, headers
        )
        return t.GenericResponse.from_dict(res or {})

    async def totp_mfa_setup(
        self, req: t.OtpMfaSetupRequest | None = None, headers: dict[str, str] | None = None
    ) -> t.AuthToken:
        data = req.to_dict() if req is not None else None
        res = await self._invoke("totp_mfa_setup", d.PUBLIC["totp_mfa_setup"], data, headers)
        return t.AuthToken.from_dict(res or {})
    ...
    async def webauthn_credentials(
        ...
        res = await self._invoke(
            "webauthn_credentials", d.PUBLIC["webauthn_credentials"], None, headers
        )
    ...
    async def get_token(self, req: t.GetTokenRequest) -> t.GetTokenResponse:
        ...
        return t.GetTokenResponse.from_dict(await self._oauth_form("/oauth/token", body))
